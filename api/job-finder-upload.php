...
try {
    ...
    if (!isset($_FILES['cv']) || !is_array($_FILES['cv']) || count($_FILES) !== 1) throw new InvalidArgumentException('missing');
    $file = $_FILES['cv'];
    ...
    $name = $file['name'] ?? '';
    if (!is_string($name) || strtolower(pathinfo($name, PATHINFO_EXTENSION)) !== 'pdf' || substr_count($name, '.') !== 1) throw new InvalidArgumentException('extension');
    ...
    $response = job_finder_call('N8N_JOB_FINDER_UPLOAD_URL', [CURLOPT_POST => true, CURLOPT_POSTFIELDS => [
        'cv' => new CURLFile($file['tmp_name'], 'application/pdf', 'lebenslauf.pdf'),
    ]]);
    ...
    $_SESSION['job_finder_profile'] = $profileId;
    $_SESSION['job_finder_profile_at'] = time();
    job_finder_json(200, ['success' => true]);
} catch (InvalidArgumentException) {
    job_finder_json(422, ['success' => false, 'message' => 'Bitte wähle eine gültige PDF mit maximal 5 MB aus.']);
} catch (Throwable $exception) {
    error_log('Job-Finder upload failed: ' . get_class($exception));
    job_finder_json(502, ['success' => false, 'message' => 'Der Lebenslauf konnte nicht verarbeitet werden.']);
}
