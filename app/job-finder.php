...
function job_finder_require_request(string $contentType = ''): void
{
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
        header('Allow: POST');
        job_finder_json(405, ['success' => false, 'message' => 'Methode nicht erlaubt.']);
    }
    if ($contentType !== '' && !str_starts_with(strtolower($_SERVER['CONTENT_TYPE'] ?? ''), $contentType)) {
        job_finder_json(415, ['success' => false, 'message' => 'Ungültiges Anfrageformat.']);
    }
    job_finder_session();
    $token = $_SERVER['HTTP_X_CSRF_TOKEN'] ?? '';
    if (!is_string($token) || !isset($_SESSION['job_finder_csrf']) || !hash_equals($_SESSION['job_finder_csrf'], $token)) {
        job_finder_json(403, ['success' => false, 'message' => 'Die Anfrage konnte nicht verifiziert werden.']);
    }
}
...
function job_finder_rate_limit(string $action, int $limit, int $window): void
{
    ...
    if (!is_dir($directory) && !@mkdir($directory, 0700, true) && !is_dir($directory)) {
        ...
        job_finder_json(503, ['success' => false, 'message' => 'Der Dienst ist momentan nicht verfügbar.']);
    }
    ...
    if (count($hits) >= $limit) {
        ...
        job_finder_json(429, ['success' => false, 'message' => 'Zu viele Anfragen. Bitte warte einen Moment und versuche es erneut.']);
    }
    $hits[] = $now;
...
function job_finder_call(string $urlName, array $options): array
{
    ...
    $secret = job_finder_config('N8N_JOB_FINDER_WEBHOOK_SECRET');
    $curl = curl_init($url);
